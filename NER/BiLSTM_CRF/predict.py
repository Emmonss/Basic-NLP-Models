...
def get_model(model_path,crf_tagsum):
    crf = CRF(crf_tagsum)
    model = load_model(model_path,custom_objects={'CRF':CRF,'get_loss':crf.get_loss})
    model.summary()
    return model
...
def predict_main(model_path,param_path,val_path,save_path_head,batch_size=32):
    ...
    model = get_model(model_path=model_path,crf_tagsum=train_param['train_tagSum'])
    IndexWordDict = reverse_dict(train_param['train_wordIndexDict'])
    IndexTagDict = reverse_dict(train_param['train_tagIndexDict'])

    _,_,_,_,_,_,_,val_X,val_y = get_words_label_data(val_path,
...
# train data mode
################################################################################################
def get_weibo_data_predict():
    model_path = './models/weibo_ner.h5'
    param_path = './models/weibo_ner_params.pkl'
    val_path = '../data/Proessdata/weibo_test.csv'
    predict_main(model_path=model_path,param_path=param_path,val_path=val_path,
                 save_path_head='weibo_test')

def get_clue_data_predict():
    model_path = './models/clue_ner.h5'
    param_path = './models/clue_ner_params.pkl'
    val_path = '../data/Proessdata/clue_dev.csv'
    predict_main(model_path=model_path,param_path=param_path,val_path=val_path,
                 save_path_head='clue_dev')


def get_msra_data_predict():
    model_path = './models/msra_ner.h5'
    param_path = './models/msra_ner_params.pkl'
    val_path = '../data/Proessdata/msra_test.csv'
    predict_main(model_path=model_path,param_path=param_path,val_path=val_path,
                 save_path_head='msra_test')
################################################################################################


if __name__ == '__main__':
    get_msra_data_predict()
    pass
