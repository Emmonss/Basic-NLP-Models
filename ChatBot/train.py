...
def train_di():
    ...
    if train_config.saving_flg:
        save_path = train_config.save_model_path_di.format(datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        model.save_weights(model_path=save_path, model_name=train_config.model_name)
        basic_params = {
            ...
            'dict_vocab_pure':train_config.dict_vocab_pure,
            'inpath' : train_config.inpath_di,
            'tarpath' : train_config.tarpath_di
        }
