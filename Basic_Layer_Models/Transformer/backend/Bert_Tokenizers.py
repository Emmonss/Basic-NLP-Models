            ...
            if t not in new_token_dict:
                ...
                if len(t)>1:
                    for c in Tokenizer.stem(t):
                        if (Tokenizer._is_cjk_charactoer(c) or
...
class TokenizerBase(object):
    ...
    def __init__(
        ...
        :param token_start:
        :param token_end:
        :param pre_tokenize: 外部传入的分词函数，用作对文本进行预分词，如果传入则先执行预分词函数
        :param token_translate: 映射字典，主要用在tokenize之后，将某些特殊的字段映射为token
        '''
        self._token_pad = '[PAD]'
        self._token_unk = '[UNK]'
...
class SpTokenizer(Tokenizer):
    ...
    def decode(self, ids):
        ...
        tokens = [
            ...
            for token in self.id_to_token(ids)
        ]
